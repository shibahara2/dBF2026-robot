...
The Dockerfiles use a small core Python image for the Flask/mock application and a CUDA aarch64 image for voice development. The devcontainer is based on the core image, installs Node.js and Codex CLI, mounts the repository at `/workspace`, and writes its Codex configuration into a named volume.
...
- Run the existing pytest suite in the core container.
